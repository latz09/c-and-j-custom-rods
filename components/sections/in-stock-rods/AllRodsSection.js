...
function FilterToggle({ filters, filter, setFilter }) {
	return (
...
export default function AllRodsSection({
	...
	const filters = FILTERS_BY_SEASON[activeSeason];
	const [filter, setFilter] = useState(activeSeason === 'ice' ? 'ice' : 'all');
	...
	return (
		<section id='all-rods' className='section-x-padding mt-5 mb-5 lg:mb-8.75 scroll-mt-3'>
			...
			<FilterToggle filters={filters} filter={filter} setFilter={setFilter} />

			<div className='grid md:grid-cols-2 lg:grid-cols-3 gap-1.5 mt-1.5 lg:mt-1.25'>
				{filtered.map((p, index) => (
					<RodCard key={p.id} product={p} seriesList={seriesList} index={index} />
				))}
			</div>
